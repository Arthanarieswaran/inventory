    ...
    if (row) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.invoice_qty >= Number($("#qty_" + row).val())) {
                    getFullTotal(row);
                } else {
                    alert('Stock quantity is exceed. Inside stock quantity ' + response.invoice_qty);
                    $("#qty_" + row).val(1)
                    getFullTotal(row);
                }
            }
    ...
    if (product_id == "") {
        ...
        $.ajax({
            ...
            success: function(response) {
                    ...
                    $("#prodDetailin_" + row_id).html(response.name + '<br>' + response.hsn);
...
function subAmount() {
    ...
    if (extra_gst) {
        var totalSubAmount = extra_gst + '%';
        var eachGst = Number(totalNetAmount) * Number(totalSubAmount);
